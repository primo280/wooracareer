...
const categories: JobCategory[] = [
  { name: "Développement", icon: "💻", jobs: 245, color: "bg-blue-500" },
  { name: "Design", icon: "🎨", jobs: 89, color: "bg-purple-500" },
  { name: "Marketing", icon: "📈", jobs: 156, color: "bg-green-500" },
  { name: "Data", icon: "📊", jobs: 78, color: "bg-orange-500" },
  { name: "Finance", icon: "💰", jobs: 134, color: "bg-emerald-500" },
  { name: "RH", icon: "👥", jobs: 67, color: "bg-pink-500" }
]
          ...
          {categories.map((category, index) => (
            <Card key={index} className="hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer group">
              <CardContent className="p-6 text-center">
                <div className={`w-12 h-12 ${category.color} rounded-lg flex items-center justify-center text-white text-xl mx-auto mb-3 group-hover:scale-110 transition-transform duration-300`}>
                  {category.icon}
                </div>
